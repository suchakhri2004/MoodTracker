...
const currentYear = yearCurrent + 543
const currentMonth = new Date().getMonth() + 1;
const startYear = parseInt(email.substring(0, 2)) + 2500;
...
if (diff >= 0 && diff < 5) {
  ...
  year = 'จบการศึกษา';
    ...
    await pool.query(`
      INSERT INTO users (office365_id, email, name, faculty, department, role, year, accesstoken, createdAt, updatedAt)
    ...
    `, [office365Id, email, name, faculty, department, role, year, response.accessToken]);
    ...
    res.json({ token:token,role,status:200 });
  } catch (error) {
...
app.post('/moodTracker', authenticateJWT, async (req: RequestWithToken, res: Response) => {
  try {
    ...
    const words = content.split(',').map((word: string) => word.trim());
    const today = format(new Date(), 'yyyy-MM-dd');

    for (const word of words) {
      const existingWord = await pool.query(
        `SELECT * FROM wordcloud WHERE word = $1 AND createdAt = $2`,
        [word, today]
      );

      if ((existingWord?.rowCount ?? 0) > 0) {
        await pool.query(
          `UPDATE wordcloud SET count = count + 1 WHERE word = $1 AND createdAt = $2`,
          [word, today]
        );
      } else {
        await pool.query(
          `INSERT INTO wordcloud (word, count, createdAt) VALUES ($1, 1, $2)`,
          [word, today]
        );
      }
    }

    res.status(200).send('Mood and words recorded successfully');
  } catch (error) {
    console.error('Error recording mood and words:', error);
...
app.get('/moodReport', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const weeklyQuery = `
      SELECT mood, COUNT(*) as count
      FROM moodtracker
      WHERE createdAt::DATE BETWEEN $1::DATE AND $2::DATE
      GROUP BY mood
    `;
    ...
    const dailyResult = await pool.query(dailyQuery, [currentDateDate]);
    const weeklyResult = await pool.query(weeklyQuery, [startOfWeekDate, endOfWeekDate]);
    const monthlyResult = await pool.query(monthlyQuery, [startOfMonthDate]);
